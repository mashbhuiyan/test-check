...
const ClickEvent = require('../../../models/click_event');
...
module.exports.index = async function (req, res, next) {
    ...
    util.dynamicReplicaDB(req).query(query, async (error, results) => {
        ...
        if (clickModel.campaign_ids.length > 0) {
            ...
            Promise.all([scheduleModifier, filterGroups, modifierGroups, adGroups]).then(async results => {
                ...
                if (process_response.listings.length > 0) {
                    ...
                    let endTime = new Date().getTime();
                    clickModel.storeClickOpportunity().then(results => {
                        clickModel.storeClickResultToDatabase(); // Store click results when non of the campaign qualified
                    });
...
module.exports.post = function (req, res) {
    ...
    db.query(`select *
        ...
        if (error) {
            ...
            res.status(500).json({return_url: null, error: 'Something went wrong!'});
        } else {
            const params = req.body ? req.body.lead : {};
            if (results.rows[0]) {
                const partner_id = req.body.token ? req.body.token.partner_id : '';
                const clickEvent = new ClickEvent(req.brand_conf, results.rows[0]);
                clickEvent.post(params, partner_id);
                res.status(200).json({return_url: clickEvent.click_listing.tracking_url});
            } else {
                res.status(404).json({return_url: null, error: 'Click listings not found'});
            }
        }
    });
}
    ...
    db.query(`select *
        ...
        if (error) {
            ...
            if (results.rows[0]) {
                const clickEvent = new ClickEvent(req.brand_conf, results.rows[0]);
                if (req.body) {
                    clickEvent.updateClickListing(req.body).then(resp => {
                        if (resp != 'post event success') {
                            console.log(resp);
                        }
                    });
                }
                clickEvent.updateAdGroupAdLogic().then(resp => {
                    clickEvent.updateCampaignAdLogic().then(resp => {
                        res.status(200).json({success: true, message: 'success'});
                    });
                });
            } else {
                res.status(404).json({success: false, message: 'Click listings not found'});
            }
        }
    });
}
...
function responseFormat(listings, objClickModel) {
    return listings.map(campaign => {
        ...
        if (objClickModel.token_type === 'admin') {
            ...
            listing_resp.retail = !campaign.enterprise;
